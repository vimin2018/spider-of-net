...
def executer(source_result,j):
    ...
    while i < len(source_url_list):
        ...
        time.sleep(1)
        # 进一步获取目标图片的大图地址
        result = requests.get(url_aim,headers=headers).text
        time.sleep(1)
        ...
        if not os.path.exists(filepath):
            downloadFile(image_name, url,filepath,i,length,r)
        else:
            if length > os.path.getsize(filepath):
                downloadFile(image_name, url,filepath,i,length,r)
        print("第",i+1,"张完成")
        #f.write("完成\n")
        i+=1
...
def downloadFile(image_name, url,filepath,i,length,r):
    f = open(filepath, 'wb')
    count = 0
    count_tmp = 0
    time1 = time.time()
...
def judge(source_result,j):
    ...
    if len(source_url_next_tag) != 0:
        ...
        print("正在进入下一页")
        time.sleep(2)
        source_result=requests.get(source_url_next,headers=headers).text
        print("已找到原网页代码")
...
if __name__== "__main__":
    ...
    #f.write(artist)
    #f.write("\n")
    source_url = "https://konachan.com/post?tags=%s" %str(artist)
    print(source_url)
    source_result = requests.get(source_url,headers=headers).text
    print("已找到原网页代码")
    # 新建该作者文件夹
    path = 'F:/%s'%str(get_message)
